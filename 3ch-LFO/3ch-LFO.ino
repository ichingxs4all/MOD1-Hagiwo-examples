...
// Each LFO has a selectable waveform (Triangle, Square, RampUp, RampDown , Random and Sine) chosen by a push button on D4 (INPUT_PULLUP).
...
static const int numberOfWaveTypes= 6;
...
void handleButtonInput() {
  ...
  if (currentMillis - buttonPreviousMillis > debounceDelay) {
    ...
    if (reading == LOW && lastButtonState == HIGH) {
      ...
      waveType = (waveType + 1) % numberOfWaveTypes;
...
// ------------------------------------------------------
// Create a ramp up wave in waveTable (8-bit range 0 or 255)
void createRampUpTable() {
    // First half rising 0->255, second half again rising from 0->255
  for (int i = 0; i < TABLE_SIZE; i++) {
    if (i < (TABLE_SIZE / 2)) {
      float val = (255.0f * i) / (TABLE_SIZE / 2);
      waveTable[i] = (uint8_t)val;
...
void createRampDownTable() {
 // First half falling from 255 ->0 , second half again falling from 255->0
 for (int i = 0; i < TABLE_SIZE; i++) {
    if (i < (TABLE_SIZE / 2)) {
      float val = 255.0f - (255.0f * i) / (TABLE_SIZE / 2);
      waveTable[i] = (uint8_t)val;
    } else {
     float val = 255.0f - (255.0f * i) / (TABLE_SIZE / 2);
      waveTable[i] = (uint8_t)val;
...
// Create a waveTable with random numbers (8-bit range 0~255)
void createRandomTable() {
  for (int i = 0; i < TABLE_SIZE; i++) {
    float randVal = random (255.0f); 
    waveTable[i] = (uint8_t)randVal;
  }
}
